 ...
 * File: Yuxuan_iterative_test_single.c
 *
...
static void my_Extract_features_Jep(
    ...
    const signed char feature_selector_struct[17], emxArray_real32_T *Features)
{
  ...
  int loop_ub;
  int n_feat_tmp;
  Rlocs_g_data = Rlocs_g->data;
  ECG_segment_g_data = ECG_segment_g->data;
  ...
  if (ctrl_vec_g[0] != 0.0F) {
    denoise_rwd(ECG_Sig, ECG_segment_g);
    ECG_segment_g_data = ECG_segment_g->data;
    ctrl_vec_g[0] = 0.0F;
  }
  emxInit_real32_T(&tHRV, 2);
  if (ctrl_vec_g[1] != 0.0F) {
    emxInit_real32_T(&r, 1);
    i = r->size[0];
    r->size[0] = ECG_segment_g->size[0];
    emxEnsureCapacity_real32_T(r, i);
    tHRV_data = r->data;
    loop_ub = ECG_segment_g->size[0] - 1;
    for (i = 0; i <= loop_ub; i++) {
      tHRV_data[i] = ECG_segment_g_data[i];
    }
    pan_tompkin_algorithm_segmented(r, tHRV, Rlocs_g);
    Rlocs_g_data = Rlocs_g->data;
    emxFree_real32_T(&r);
    ctrl_vec_g[1] = 0.0F;
  }
  i = tHRV->size[0] * tHRV->size[1];
  tHRV->size[0] = 1;
  tHRV->size[1] = Rlocs_g->size[1];
  emxEnsureCapacity_real32_T(tHRV, i);
  tHRV_data = tHRV->data;
  loop_ub = Rlocs_g->size[0] * Rlocs_g->size[1] - 1;
  for (i = 0; i <= loop_ub; i++) {
    tHRV_data[i] = Rlocs_g_data[i];
  }
  unique_vector(tHRV, Rlocs_g);
  Rlocs_g_data = Rlocs_g->data;
  emxInit_real32_T(&locs, 2);
  i = locs->size[0] * locs->size[1];
  locs->size[0] = 1;
  locs->size[1] = Rlocs_g->size[1];
  emxEnsureCapacity_real32_T(locs, i);
  locs_data = locs->data;
  loop_ub = Rlocs_g->size[1];
  for (i = 0; i < loop_ub; i++) {
    locs_data[i] = Rlocs_g_data[i] / 256.0F;
  }
  for (i = 0; i < 400; i++) {
    RR_Interv[i] = rtNaNF;
  }
  ...
  while ((b_i < 350) && (!c_isnan(RR_Interv[b_i + 50]))) {
    ...
    ECG_segment_g_data = feature->data;
    for (i1 = 0; i1 < loop_ub; i1++) {
      ECG_segment_g_data[i1] = rtNaNF;
    }
    /* Lorenz features */
    if ((idx9_tmp != -1) || (idx10_tmp != -1) || (idx11_tmp != -1) ||
      ...
      if (idx10_tmp != -1) {
        ECG_segment_g_data[idx10_tmp - 1] =
            Feature10(CSI_filt_tmp_data, Slope_tmp);
        /* f10 */
      }
      if (idx11_tmp != -1) {
        ECG_segment_g_data[idx11_tmp - 1] =
            Feature10(Mod_CSI_filt_tmp_data, Slope_tmp);
        /* f11 */
      }
      if (idx12_tmp != -1) {
        ECG_segment_g_data[idx12_tmp - 1] = Feature10(CSI_tmp_data, Slope_tmp);
        /* f12 */
      }
      if (idx13_tmp != -1) {
        ECG_segment_g_data[idx13_tmp - 1] =
            Feature10(Mod_CSI_tmp_data, Slope_tmp);
        /* f13 */
      }
      if (idx14_tmp != -1) {
        ECG_segment_g_data[idx14_tmp - 1] = Feature14(CSI_tmp_data);
        /* f14 */
      }
      if (idx15_tmp != -1) {
        ECG_segment_g_data[idx15_tmp - 1] = Feature14(Mod_CSI_tmp_data);
        /* f15 */
      }
      /*               */
      /*          end */
      if (idx9_tmp != -1) {
         ...
         * %f9 */
        ECG_segment_g_data[idx9_tmp - 1] = Feature14(HR_diff_orig_tmp_data);
        /* f9 */
      }
    }
    ...
    if ((idx5_tmp != -1) || (idx6_tmp != -1) || (idx7_tmp != -1) ||
      ...
      if (idx5_tmp != -1) {
        ECG_segment_g_data[idx5_tmp - 1] = Slope_tmp;
        /* f5 */
      }
      if (idx6_tmp != -1) {
        ECG_segment_g_data[idx6_tmp - 1] = LF_tmp;
        /* f6   */
      }
      if (idx7_tmp != -1) {
        ECG_segment_g_data[idx7_tmp - 1] = HF_tmp;
        /* f7   */
      }
      if (idx8_tmp != -1) {
        ECG_segment_g_data[idx8_tmp - 1] = LF_HF_tmp;
        /* f8   */
      }
    }
    if ((idx1_tmp != -1) || (idx2_tmp != -1) || (idx3_tmp != -1) ||
      ...
      if (idx1_tmp != -1) {
        ECG_segment_g_data[idx1_tmp - 1] = Slope_tmp;
        /* f1 */
      }
      if (idx2_tmp != -1) {
        ECG_segment_g_data[idx2_tmp - 1] = LF_tmp;
        /* f1 */
      }
      if (idx3_tmp != -1) {
        ECG_segment_g_data[idx3_tmp - 1] = HF_tmp;
        /* f3 */
      }
      if (idx4_tmp != -1) {
        ECG_segment_g_data[idx4_tmp - 1] = LF_HF_tmp;
        /* f4 */
      }
    }
    if (idx16_tmp != -1) {
      ECG_segment_g_data[idx16_tmp - 1] =
          Feature16(Spo2_sig, Rlocs_g, (float)b_i + 51.0F);
    }
    /*      if feature_selector_struct.mask(6) */
    if (idx17_tmp != -1) {
      ECG_segment_g_data[idx17_tmp - 1] =
          Feature17(Spo2_sig, Rlocs_g, (float)b_i + 51.0F);
    }
    /*  switch n_feat */
    /*      case 2 */
    ...
    tHRV_data = b_Features->data;
    for (i1 = 0; i1 < loop_ub; i1++) {
      for (i2 = 0; i2 < input_sizes_idx_0; i2++) {
        tHRV_data[i2 + b_Features->size[0] * i1] =
            Features_data[i2 + input_sizes_idx_0 * i1];
      }
    }
    for (i1 = 0; i1 < loop_ub; i1++) {
      for (i2 = 0; i2 < sizes_idx_0_tmp; i2++) {
        tHRV_data[input_sizes_idx_0 + b_Features->size[0] * i1] =
            ECG_segment_g_data[sizes_idx_0 * i1];
      }
    }
 ...
 * Return Type  : void
 */
void Yuxuan_iterative_test_single(const emxArray_real32_T *ECG_sig,
                                  const emxArray_real32_T *Spo2_sig,
...
{
  emxArray_real32_T *b_extract_feature;
  emxArray_real32_T *c_extract_feature;
  emxArray_real32_T *d_extract_feature;
  emxArray_real32_T *e_extract_feature;
  emxArray_real32_T *extract_feature;
  emxArray_real32_T *f_extract_feature;
  emxArray_real32_T *r;
  ...
  float *b_extract_feature_data;
  float *extract_feature_data;
  float *r5;
  int b_ind;
  int i;
  boolean_T exitg1;
  if (!isInitialized_Yuxuan_iterative_test_single) {
  ...
  *ind = 1.0F;
  b_ind = 0;
  emxInit_real32_T(&extract_feature, 2);
  emxInit_real32_T(&r, 2);
  emxInit_real32_T(&r1, 2);
  emxInit_real32_T(&r2, 2);
  emxInit_real32_T(&r3, 2);
  emxInit_real32_T(&r4, 2);
  emxInit_real32_T(&b_extract_feature, 2);
  emxInit_real32_T(&c_extract_feature, 2);
  emxInit_real32_T(&d_extract_feature, 2);
  emxInit_real32_T(&e_extract_feature, 2);
  emxInit_real32_T(&f_extract_feature, 2);
  exitg1 = false;
  while ((!exitg1) && (b_ind < 5)) {
    ...
    *ind = (float)b_ind + 1.0F;
    ...
    if (sum(b_masks) == 0.0) {
      ...
      my_Extract_features_Jep(ECG_sig, Spo2_sig, b_masks, extract_feature);
      extract_feature_data = extract_feature->data;
      /*  ----------- test with training models ------------ */
      switch (b_ind + 1) {
      case 1:
        repmat(extract_feature->size[0], r1);
        r5 = r1->data;
        if ((extract_feature->size[0] == r1->size[0]) &&
            (extract_feature->size[1] == 4)) {
          i = c_extract_feature->size[0] * c_extract_feature->size[1];
          c_extract_feature->size[0] = extract_feature->size[0];
          c_extract_feature->size[1] = 4;
          emxEnsureCapacity_real32_T(c_extract_feature, i);
          b_extract_feature_data = c_extract_feature->data;
          predict_res = extract_feature->size[0] * 4;
          for (i = 0; i < predict_res; i++) {
            b_extract_feature_data[i] = extract_feature_data[i] / r5[i];
          }
          predict_res = test_classifier_s1(c_extract_feature, expl_temp.Prior,
                                           thrshd, &conf_seiz, &conf_gray);
        } else {
          predict_res = binary_expand_op_3(extract_feature, r1, &expl_temp,
                                           thrshd, &conf_seiz, &conf_gray);
        }
        break;
      case 2:
        f_repmat(extract_feature->size[0], r2);
        r5 = r2->data;
        if ((extract_feature->size[0] == r2->size[0]) &&
            (extract_feature->size[1] == 11)) {
          i = d_extract_feature->size[0] * d_extract_feature->size[1];
          d_extract_feature->size[0] = extract_feature->size[0];
          d_extract_feature->size[1] = 11;
          emxEnsureCapacity_real32_T(d_extract_feature, i);
          b_extract_feature_data = d_extract_feature->data;
          predict_res = extract_feature->size[0] * 11;
          for (i = 0; i < predict_res; i++) {
            b_extract_feature_data[i] = extract_feature_data[i] / r5[i];
          }
          predict_res = test_classifier_s2(
              d_extract_feature, b_expl_temp.IsCached, b_expl_temp.Prior,
              thrshd, &conf_seiz, &conf_gray);
        } else {
          predict_res = binary_expand_op_2(extract_feature, r2, &b_expl_temp,
                                           thrshd, &conf_seiz, &conf_gray);
        }
        break;
      case 3:
        g_repmat(extract_feature->size[0], r3);
        r5 = r3->data;
        if ((extract_feature->size[0] == r3->size[0]) &&
            (extract_feature->size[1] == 12)) {
          i = e_extract_feature->size[0] * e_extract_feature->size[1];
          e_extract_feature->size[0] = extract_feature->size[0];
          e_extract_feature->size[1] = 12;
          emxEnsureCapacity_real32_T(e_extract_feature, i);
          b_extract_feature_data = e_extract_feature->data;
          predict_res = extract_feature->size[0] * 12;
          for (i = 0; i < predict_res; i++) {
            b_extract_feature_data[i] = extract_feature_data[i] / r5[i];
          }
          predict_res = test_classifier_s3(
              e_extract_feature, c_expl_temp.IsCached, c_expl_temp.Prior,
              thrshd, &conf_seiz, &conf_gray);
        } else {
          predict_res = binary_expand_op_1(extract_feature, r3, &c_expl_temp,
                                           thrshd, &conf_seiz, &conf_gray);
        }
        break;
      case 4:
        h_repmat(extract_feature->size[0], r4);
        r5 = r4->data;
        if ((extract_feature->size[0] == r4->size[0]) &&
            (extract_feature->size[1] == 13)) {
          i = f_extract_feature->size[0] * f_extract_feature->size[1];
          f_extract_feature->size[0] = extract_feature->size[0];
          f_extract_feature->size[1] = 13;
          emxEnsureCapacity_real32_T(f_extract_feature, i);
          b_extract_feature_data = f_extract_feature->data;
          predict_res = extract_feature->size[0] * 13;
          for (i = 0; i < predict_res; i++) {
            b_extract_feature_data[i] = extract_feature_data[i] / r5[i];
          }
          predict_res = test_classifier_s4(
              f_extract_feature, d_expl_temp.IsCached, d_expl_temp.Prior,
              thrshd, &conf_seiz, &conf_gray);
        } else {
          predict_res = binary_expand_op(extract_feature, r4, &d_expl_temp,
                                         thrshd, &conf_seiz, &conf_gray);
        }
        break;
      default:
        i_repmat(extract_feature->size[0], r);
        r5 = r->data;
        if ((extract_feature->size[0] == r->size[0]) &&
            (extract_feature->size[1] == 17)) {
          i = b_extract_feature->size[0] * b_extract_feature->size[1];
          b_extract_feature->size[0] = extract_feature->size[0];
          b_extract_feature->size[1] = 17;
          emxEnsureCapacity_real32_T(b_extract_feature, i);
          b_extract_feature_data = b_extract_feature->data;
          predict_res = extract_feature->size[0] * 17;
          for (i = 0; i < predict_res; i++) {
            b_extract_feature_data[i] = extract_feature_data[i] / r5[i];
          }
          predict_res = test_classifier_s5(b_extract_feature, e_expl_temp.Prior,
                                           thrshd, &conf_seiz, &conf_gray);
        } else {
          predict_res = binary_expand_op_4(extract_feature, r, &e_expl_temp,
                                           thrshd, &conf_seiz, &conf_gray);
        }
        break;
      }
      ...
      mask_runtime[b_ind] = (c[4] * 60.0F + c[5]) - mask_start_time;
  ...
  emxFree_real32_T(&f_extract_feature);
  emxFree_real32_T(&e_extract_feature);
  emxFree_real32_T(&d_extract_feature);
  emxFree_real32_T(&c_extract_feature);
  emxFree_real32_T(&b_extract_feature);
  emxFree_real32_T(&r4);
  emxFree_real32_T(&r3);
  emxFree_real32_T(&r2);
  emxFree_real32_T(&r1);
  emxFree_real32_T(&r);
  emxFree_real32_T(&extract_feature);
  b_clock(dv);
  for (i = 0; i < 6; i++) {
    c[i] = (float)dv[i];
  }
  *runtime = (c[4] * 60.0F + c[5]) - iterative_start_time;
...
/*
 * File trailer for Yuxuan_iterative_test_single.c
 *
 * [EOF]
 */
